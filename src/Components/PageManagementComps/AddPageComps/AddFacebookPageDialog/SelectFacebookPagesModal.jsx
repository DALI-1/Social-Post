...
export default function PagesDialog(props) {
  ...
  const responseFacebook = (response) => {
    ...
    fetch(
      `https://graph.facebook.com/v16.0/me/accounts?fields=name,id,instagram_business_account{username,id},access_token&&access_token=${response.accessToken}`
    )
    ...
    variables.Pages.FBSelectPagesList.data.map((Page) => {
      ...
      if (checkbox.checked) {
        ...
        if (Page.instagram_business_account != undefined) {
          List_Of_FacebookPages_With_Possible_BusinessAccounts = [
            ...List_Of_FacebookPages_With_Possible_BusinessAccounts,
            Page,
          ];
        }
  ...
  return (
    <>
      ...
      >
        <DialogTitle id="alert-dialog-title">Facebook Pages Add</DialogTitle>
        <DialogContent>
          <DialogContentText className="m-2" id="alert-dialog-description">
            Please Select the Facebook pages you would like to add within your
            current group.
            <br></br>
            <strong>
              Note: Only Facebook pages allowed, your account will not be shown
              here.
            </strong>
          </DialogContentText>
          ...
          {PagesLoadedFlag &&
            variables.Pages.FBSelectPagesList.data.length == 0 && (
              <strong>
                No Pages found or the pages you own are all already added.
              </strong>
            )}
