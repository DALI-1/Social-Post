...
export default function PagesDialog(props) {
  ...
  const getInstagramPages = async (access_token) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://graph.facebook.com/v11.0/me/accounts?fields=name,id,instagram_business_account{username,id},access_token&access_token=${access_token}`
      );
      ...
      data.data.map((page) => {
        if (page.instagram_business_account != undefined) {
          ...
          variables.Pages.CurrentGroupPages.map((Existing_Page) => {
            ...
            if (
              Existing_Page.platformPageID == page.instagram_business_account.id
            ) {
  ...
  useEffect(() => {
    ...
    if (isSdkLoaded == true) FacebookLoginRefButton.current.click();
  }, [isSdkLoaded]);
  ...
  const handleAddPage = () => {
    ...
    variables.Pages.INGSelectPagesList.map((Page) => {
      var checkbox = document.getElementById(
        'NEWINSTAPage' + Page.instagram_business_account.id
      );
  ...
  return (
    <>
      ...
      >
        <DialogTitle id="alert-dialog-title">Instagram Pages Add</DialogTitle>
        <DialogContent>
          <DialogContentText className="m-2" id="alert-dialog-description">
            Please Select the instagram accounts you would like to add within
            your current group.
            <br></br>
            <br></br>
            <strong>Note: Only instagram businesss accounts allowed</strong>, if
            you have a personal account please link it to a Facebook Page and
            change it from personal to Business.
          </DialogContentText>
          <FacebookLogin
          ...
          {PagesLoadedFlag && pages.length == 0 && (
            <strong>
              {' '}
              No Pages found or the pages you own already added within the group
              you currently in.
            </strong>
          )}
          {PagesLoadedFlag &&
            pages.map((Page, index) => {
              return (
                <Form.Check
                  id={'NEWINSTAPage' + Page.instagram_business_account.id}
                  key={'FBPage' + Page.instagram_business_account.id}
                  type="switch"
