...
const handleAddINPages=(()=>{
  ...
  if(variables.Pages.SelectedPlatformID==1)
  {
    ...
    var JsonObject={
      ...
      "ownerFB_shortLivedToken": variables.FacebookUser.LoggedFacebookUserInfo.accessToken,
      "listOfPages": []
...
//This Flag indicate if he selected the optional Facebook Page of the current Insta Page or not, by default its false until we found it in the INsta optional facebook list
      ...
      variables.Pages.FBINGSelectedOptionalPagesList.map((SelectedOptionalFBKPage)=>{
            if(SelectedOptionalFBKPage.id==Selected_Facebook_Page.id)
            {
              listOfPages=[...listOfPages,{"pageID":Selected_Facebook_Page.id ,"page_shortLivedToken":Selected_Facebook_Page.access_token , "associatedPageID":Selected_Facebook_Page.instagram_business_account.id}]
     ...
     //if no optional Facebook page added we just add the Insta page
      ...
      {
        listOfPages=[...listOfPages,{"pageID":Selected_Facebook_Page.id ,"page_shortLivedToken":Selected_Facebook_Page.access_token , "associatedPageID":"null"}]
      }
    ...
    let url2=process.env.REACT_APP_BACKENDURL+process.env.REACT_APP_ADDFACEBOOKPAGE
    ...
    APIResult.then((result)=>
    {
      ...
      {
        toast.info("The One of the Facebook pages you Selected Already exist!", {
          position: "bottom-left",
          autoClose: 5000,
          hideProgressBar: false,
          closeOnClick: true,
  ...
  if(variables.Pages.SelectedPlatformID==2)
  {
    ...
    var JsonObject={
      ...
      "ownerFB_shortLivedToken": variables.FacebookUser.LoggedFacebookUserInfo.access_token,
      "listOfPages": []
...
//This Flag indicate if he selected the optional Facebook Page of the current Insta Page or not, by default its false until we found it in the INsta optional facebook list
      var OptionalSelected=false
      variables.Pages.INGFBSelectedOptionalPagesList.map((SelectedOptionalINSTAPAGE)=>{
            if(SelectedOptionalINSTAPAGE.id==SelectedINSTAPAGE.id)
            {
              listOfPages=[...listOfPages,{"pageID":SelectedINSTAPAGE.instagram_business_account.id ,"page_shortLivedToken":SelectedINSTAPAGE.access_token , "associatedPageID":SelectedINSTAPAGE.id}]
              OptionalSelected=true
            }
      })
     //if no optional Facebook page added we just add the Insta page
      if(OptionalSelected==false)
      {
        listOfPages=[...listOfPages,{"pageID":SelectedINSTAPAGE.instagram_business_account.id ,"page_shortLivedToken":SelectedINSTAPAGE.access_token , "associatedPageID":"null"}]
      }

     }) 
     JsonObject.listOfPages=listOfPages 
    let JsonObjectToSend=JSON.stringify(JsonObject)  
    let url2=process.env.REACT_APP_BACKENDURL+process.env.REACT_APP_ADDINSTAGRAMPAGE
    let UserToken=window.localStorage.getItem("AuthToken")
    let APIResult=CALL_API_With_JWTToken(url2,JsonObjectToSend,UserToken)
    Dispatch({type:variables.HeaderSpinnerActions.TurnOnRequestSpinner})
    APIResult.then((result)=>
    {
      ...
      {
        toast.success("The Instagram pages you Selected Added Successfully!", {
          position: "bottom-left",
          autoClose: 5000,
          hideProgressBar: false,
          closeOnClick: true,
      ...
      if(result.result=="Selected_Page_Exist")
      {
        toast.info("The One of the Instagram pages you Selected Already exist!", {
          position: "bottom-left",
          autoClose: 5000,
          hideProgressBar: false,
          closeOnClick: true,
  ...
  return (
    <>
      ...
      <MDBBtn outline className='mx-2 m-2' color='secondary'onClick={HandlePageDelete} >Delete Page </MDBBtn>
       </div>
       </Paper>
          </Col>
        </Row>
...
{SelectFBPageModalFlag&&<SelectFacebookPagesModal SetSelectFBPageModalFlag={SetSelectFBPageModalFlag} SetShowFBINChoiceModal={SetShowFBINChoiceModal} SetFBINPages={SetFBINPages} handleAddINPages={handleAddINPages}/>}
{ShowFBINChoiceModal&&FBINPages.length>0 &&<OptionalFBINSelectModal  handleAddINPages={handleAddINPages} SetSelectFBPageModalFlag={SetSelectFBPageModalFlag}  SetShowFBINChoiceModal={SetShowFBINChoiceModal} FBINPages={FBINPages} />}


{/*Here is  a popup that's gonna show when the user chooses to use Instagram platform to load a page */}
{SelectINSTAPageModalFlag&&<SelectInstagramPagesModal handleAddINPages={handleAddINPages} SelectINSTAPageModalFlag={SetSelectINSTAPageModalFlag} SetShowINFBChoiceModal={SetShowINFBChoiceModal} SetINFBPages={SetINFBPages}/>}
{ShowINFBChoiceModal&&INFBPages.length>0 &&<OptionalINFBSelectModal  handleAddINPages={handleAddINPages}SelectINSTAPageModalFlag={SetSelectINSTAPageModalFlag}  SetShowINFBChoiceModal={SetShowINFBChoiceModal} INFBPages={INFBPages} />}
